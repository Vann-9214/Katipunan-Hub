...
import { X, MessageCircle, CheckCircle2, RefreshCcw, Lock, ChevronLeft, User as UserIcon } from "lucide-react";
          ...
          <div className="flex w-full h-full">
            <div className="w-1/2 h-full bg-gray-100 relative overflow-hidden">
              <div className="absolute top-6 right-6 z-10">
                {post.status === "Resolved" ? (
                  ...
                  <span className={`px-4 py-2 rounded-full font-bold text-sm shadow-md ${post.type === "Found" ? "bg-[#E6C200] text-black" : "bg-[#800000] text-white"}`}>
                    {post.type}
                  </span>
                )}
              </div>
            ...
            <div className="w-1/2 h-full p-10 flex flex-col relative">
              <div className="mb-8 mt-4">
                <h2 className="text-4xl font-extrabold text-gray-900 leading-tight mb-4">{post.title}</h2>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-xl shadow-inner">👤</div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">Posted By</p>
                    <p className="text-sm font-bold text-gray-800">{post.postedBy}</p>
                  </div>
                </div>
              </div>

              <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                <div><p className="text-[#800000] font-bold text-sm mb-1">Category:</p><p className="text-gray-700 text-lg">{post.category}</p></div>
                <div><p className="text-[#800000] font-bold text-sm mb-1">Location:</p><p className="text-gray-700 text-lg">{post.location}</p></div>
                <div><p className="text-[#800000] font-bold text-sm mb-1">{post.type === "Lost" ? "Lost on:" : "Found on:"}</p><p className="text-gray-700 text-lg">{post.lostOn}</p></div>
                <div><p className="text-[#800000] font-bold text-sm mb-1">Description:</p><p className="text-gray-600 leading-relaxed whitespace-pre-line">{post.description}</p></div>
              </div>
          ...
          <div className="w-full h-full p-8 flex flex-col bg-white">
            <div className="flex items-center gap-4 mb-6 border-b pb-4 pt-2">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Messages for {post.title}</p>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
              {post.inquiries && post.inquiries.length > 0 ? (
                post.inquiries.map((inquiry) => (
                  <div key={inquiry.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100 hover:shadow-md transition-all">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-[#800000]/10 rounded-full flex items-center justify-center text-[#800000]">
                        <UserIcon size={20} />
                      </div>
                      <div>
                        <h4 className="font-bold text-gray-800">{inquiry.userName}</h4>
                        <p className="text-sm text-gray-500 truncate max-w-[200px]">{inquiry.messagePreview}</p>
                      </div>
                    </div>
