...
interface SignInFormProps {
  onClose?: () => void;
  onLoginSuccess?: () => void; // ✅ now optional
}

export default function SignInForm({ onClose, onLoginSuccess }: SignInFormProps) {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");

  // Zero-arg submit function
  const handleSubmit = () => {
      ...
      <div className="flex w-[90%] max-w-[1200px] h-[min(90vh,770px)] bg-white rounded-[20px] shadow-md relative overflow-hidden">
        ...
        <div className="bg-[linear-gradient(to_bottom,#800000,#5A0505)] w-[530px] max-h-full m-[10px] rounded-[20px] p-10">
          <h1
            ...
            style={{ fontFamily: "Montserrat, sans-serif", fontSize: "clamp(18px,4.5vw,32px)" }}
          >
            Your Gateway to Connection and Growth.
          </h1>
          <p
            className="text-white select-none mt-3"
            style={{ fontFamily: "PT sans, sans-serif", fontSize: "clamp(12px,4.5vw,20px)" }}
          >
        ...
        <div className="ml-8 mt-8 flex flex-col flex-1 p-8">
          <div>
            <Logo />
            <p className="text-[30px] select-none mt-2" style={{ fontFamily: "Montserrat, sans-serif" }}>
              Welcome, Teknoy!
            </p>
          ...
          >
            ...
            <div className="pt-2">
              <Button text="Log in" width="w-full" onClick={handleSubmit} />
            </div>
          </form>
        </div>
        ...
        {/* Close button */}
        <div>
          <TextButton text="X" onClick={onClose} className="absolute top-3 right-3" />
        </div>
      </div>
    </div>
  );
