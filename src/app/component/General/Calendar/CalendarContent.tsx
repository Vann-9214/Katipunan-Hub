...
useEffect(() => {
  ...
  function getPhilippineHolidays(y: number): Holiday[] {
    ...
    base.push({ name: "Maundy Thursday", month: maundy.getMonth() + 1, day: maundy.getDate() });
    base.push({ name: "Good Friday", month: goodFriday.getMonth() + 1, day: goodFriday.getDate() });

    // National Heroes Day -> last Monday in August
    const lastMonAug = lastMondayOfMonth(y, 7); // 0-based index (7 => Aug)
    base.push({ name: "National Heroes Day", month: lastMonAug.getMonth() + 1, day: lastMonAug.getDate() });
  ...
  const holidaysForCurrentMonth = holidaysForYear.filter(h => h.month === currentDate.getMonth() + 1);
    ...
    <div className="relative min-h-screen bg-white flex flex-col">
      ...
      {menuOpen && (
        <div
          ...
          style={{ left: "1150px", top: "250px", width: "217px", borderRadius: 25, backgroundColor: "#D2C8CA" }}
        >
          {[
            ...
            <div
              ...
              className={`flex items-center gap-2 w-full px-3 py-2 rounded-md cursor-pointer ${ptSans.className} text-[17px] ${selectedMenu === item.name ? "bg-black/20" : "bg-transparent"}`}
      ...
      <div className="relative flex flex-col px-[66px] pt-[130px]">
        <h1 className={`${montserrat.className} text-[#800000] text-[32px] font-extrabold`}>YOUR EVENT BOARD:</h1>

        {/* Calendar Section */}
        <div className="relative mt-[20px] w-[922px]">
          <Image src="/Calendar Box.svg" alt="Calendar Box" width={922} height={650} className="absolute top-0 left-0 -z-10 select-none" />

          {/* Header */}
          <div className={`${montserrat.className} flex justify-between items-center px-10 py-3 bg-[#800000] text-white text-[24px] font-semibold rounded-t-md`}>
            <button onClick={prevMonth} className="hover:text-[#FFD700] transition"><ChevronLeft size={28} /></button>
            <span>{viewMode === "month" ? `${monthName} ${year}` : year}</span>
            <button onClick={nextMonth} className="hover:text-[#FFD700] transition"><ChevronRight size={28} /></button>
          </div>

          {/* Month weekdays header */}
          {viewMode === "month" && (
            <div className={`${ptSans.className} grid grid-cols-7 text-center font-bold text-gray-700 mt-3 text-[15px]`}>
              {["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"].map((d, i) => <div key={i}>{d}</div>)}
            </div>
          )}
            ...
            <div className="grid grid-cols-7 gap-4 mt-3 px-6">
              {daysArray.map((day, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`relative w-[85px] h-[85px] flex items-center justify-center ${day ? "cursor-pointer" : "opacity-30"}`}
                    onClick={() => day && setSelectedDay(day)}
                  >
                    {isToday && <div className="absolute inset-0 border-4 border-[#FFD700] rounded-md pointer-events-none" />}
                    <Image src="/Date Box.svg" alt="Date Box" width={85} height={85} className="absolute top-0 left-0 select-none" />
                    {day && (
                      <span className={`${ptSans.className} text-[14px] font-bold absolute top-[8px] left-1/2 -translate-x-1/2 ${selectedDay === day ? "text-[#FFD700]" : "text-[#800000]"}`}>
                        {day}
                      </span>
                    )}
                  </div>
          ...
          {viewMode === "year" && (
            <div className="grid grid-cols-3 gap-4 p-4">
              {Array.from({ length: 12 }, (_, i) => (
                <div key={i}>{/* reuse mini-month if desired */}{/* You can integrate renderMiniMonth here if needed */}</div>
              ))}
            </div>
          )}
        ...
        >
          ...
          <div className="flex justify-between items-center mb-3 flex-shrink-0">
            <span className="text-[15px] text-gray-700 font-semibold">
              {selectedDay ? `${selectedDay} ${monthName.slice(0,3)}` : `${todayDate} ${monthName.slice(0,3)}`}
            </span>
            <div className="flex items-center justify-center text-[14px] font-semibold text-black rounded-full" style={{ backgroundColor: "#D9D9D9", width: "120px", height: "26px" }}>
              All reminders
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-2">
            {reminders.length === 0 ? (
              <div className="text-[14px] text-gray-500 italic">No reminders scheduled</div>
            ) : (
              reminders.map((r, i) => (
                <div key={i} className="flex items-center justify-between bg-[#D9D9D9] px-3 py-2 rounded-full text-[14px] font-medium text-[#800000]">
                  <span className="truncate">{r}</span>
                  <button onClick={() => setReminders(prev => prev.filter((_, idx) => idx !== i))} className="text-[#800000] hover:text-red-600 font-bold text-[16px] ml-2">×</button>
                </div>
              ))
            )}
          </div>
          ...
          <div className="flex items-center justify-between bg-[#D9D9D9] rounded-full px-3 mt-3 flex-shrink-0" style={{ width: "100%", height: "36px" }}>
            <input
              type="text"
              value={newReminder}
            ...
            />
            <button onClick={() => { if (newReminder.trim() !== "") { setReminders(prev => [...prev, newReminder]); setNewReminder(""); } }} className="flex items-center justify-center rounded-full bg-gray-300 hover:bg-gray-400 transition" style={{ width: 32, height: 32 }}>
              <Image src="/Bellplus.svg" alt="Add Reminder" width={18} height={18} />
            </button>
          </div>
        </div>
      )}
