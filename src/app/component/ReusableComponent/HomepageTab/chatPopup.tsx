...
// --- Types ---
// Interface for the data fetched for the component
...
// Single Conversation Item Component for the Popup (Original Component, now updated)
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-end h-full justify-between pt-1 pb-1">
        ...
        {isActive ? (
          // Show unread count badge instead of the MoreHorizontal icon
...
export default function ChatPopup() {
  ...
  useEffect(() => {
    ...
    const fetchConversationsData = async (userId: string) => {
      ...
      // 3. Fetch last message content and unread count for ALL conversations
      ...
      const conversationPromises = convoData.map(async (convo) => {
        ...
        const { count: unreadCount } = await supabase
          ...
          .eq("sender_id", otherUserId) // Message sent BY the other user
          .eq("conversation_id", convo.id)
          .is("read_at", null); // Message has NOT been read by me
  ...
  return (
    <div className="w-80 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden transform transition-all duration-300 z-40">
      ...
      {/* Footer Link - Replaced Link with a button */}
      <button
        onClick={handleSeeAllChats}
